...
async def register_user(user: schemas.UserCreate, session: AsyncSession = Depends(db_helper.get_db)):
    """Регистрирует нового пользователя."""

    db_user = await crud_user.get_user_by_username(session=session, username=user.username)
...
async def login_for_access_token(
    form_data: OAuth2PasswordRequestForm = Depends(), session: AsyncSession = Depends(db_helper.get_db)
):
